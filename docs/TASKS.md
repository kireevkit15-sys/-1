...
- Разработчик 2 ждёт Docker Compose от Lead для локальной БД
...
#### Зависимости
- Разработчик 1 ждёт от Разработчика 2 эндпоинты auth для подключения NextAuth
...
#### Зависимости
- Разработчик 1 ждёт от Lead WebSocket gateway для подключения клиента (начало недели)
- Lead ждёт от Разработчика 2 сервис вопросов для BattleService
- **Критично:** Lead должен завершить gateway к среде, чтобы Разработчик 1 мог тестировать UI
...
- Разработчик 1 ждёт от Lead матчмейкинг для кнопки «Найти соперника»
- Разработчик 2 работает параллельно (статы + контент)
...
#### Зависимости
- Разработчик 1 может начинать UI с моковыми данными, не ожидая backend
- Разработчик 2 должен завершить API к четвергу для интеграции с фронтом
...
#### Зависимости
- Lead пишет `ai.service.ts` → Разработчик 2 использует его в контроллере
- **Критично:** Lead должен завершить ai.service.ts к среде, чтобы Разработчик 2 мог подключить контроллер
...
#### Зависимости
- Разработчик 1 зависит от Lead (ai.service) и Разработчика 2 (ai.controller) — оба должны быть готовы к понедельнику
- Разработчик 2 использует `WarmupService` от Lead
...
#### Зависимости
- Разработчик 1 ждёт от Lead push-сервис для подключения на клиенте
- Lead пишет `notification.service.ts` → Разработчик 2 пишет контроллер и БД
...
#### Зависимости
- Lead генерирует вопросы → Разработчик 2 загружает их в БД
- Разработчик 1 и Разработчик 2 работают параллельно (фронт + бэк админки)
...
#### Зависимости
- Разработчик 1 ждёт от Разработчика 2 API лидерборда
...
4. Второй ревьюер: Разработчик 1 для frontend PR, Разработчик 2 для backend PR (по желанию)
...
Неделя 1: Монорепо инициализация (Lead)
    ↓
Неделя 2: Стейт-машина батла (Lead) + Auth API (Разработчик 2)
    ↓
Неделя 3: WebSocket gateway (Lead) + Battle UI (Разработчик 1)
    ↓
Неделя 4: Матчмейкинг (Lead) + Первый деплой (Lead)
    ↓
Неделя 5-6: Модули обучения API (Разработчик 2) + UI (Разработчик 1)
    ↓
Неделя 6-7: AI Service (Lead) + AI Controller (Разработчик 2) + Chat UI (Разработчик 1)
    ↓
Неделя 7-8: Warmup API (Разработчик 2) + Главный экран (Разработчик 1)
    ↓
Неделя 9-10: 500+ вопросов (Lead + Разработчик 2)
    ↓
...
| Неделя | Lead | Разработчик 1 | Разработчик 2 | Milestone |
|--------|------|---------------|---------------|-----------|
