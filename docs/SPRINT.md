...
| Legal.4 | GDPR-export (N5: `/users/me/export` — уже есть B19.7). Legal-страница «Как удалить аккаунт» + endpoint `DELETE /users/me` | backend-done | `apps/api/src/user/user.controller.ts`, `apps/api/src/user/user.service.ts`, `apps/api/src/auth/auth.service.ts`, `apps/api/test/user-delete.e2e-spec.ts` (backend ✅); `apps/web/app/(main)/settings/page.tsx` (UI — Бонди) |
...
| Launch.4 | Telegram-канал community + bot команды (`/help`, `/stats`, `/leaderboard`). Приветственное сообщение с правилами и ссылкой на /privacy | backend-done | `apps/api/src/telegram/telegram-command.service.ts`, `apps/api/src/telegram/telegram.update.ts`, `apps/api/src/telegram/telegram-notification.service.ts`, `apps/api/src/telegram/telegram.module.ts`, `apps/api/src/telegram/__tests__/telegram-command.service.spec.ts` (backend ✅, 11 unit-тестов); канал/bot token/регистрация команд через BotFather — ops-шаг (Lead) |
...
| # | Задача | Статус | Файлы | Блокер |
|---|--------|--------|-------|--------|
| L21.1 | Засидить все 184 концепта из content/sources/ в таблицу Concept с привязкой к таксономии (ветка, категория, подкатегория) | done | `scripts/seed-concepts.ts` | L20.5 |
| L21.2 | AI batch job: для каждой категории определить порядок пререквизитов между концептами → ConceptRelation записи | done | `scripts/build-knowledge-graph.ts` | L21.1 |
| L21.3 | AI batch job: для каждого концепта найти 2-5 научных подкреплений (исследования, эксперименты, статистика) → переписать голосом РАЗУМ | done | `scripts/generate-depth-layers.ts` | L21.1 |
| L21.4 | AI batch job: для каждого концепта найти 1-3 связанных книги/идеи → ключевая мысль, переписанная голосом РАЗУМ | done | `scripts/generate-depth-layers.ts` | L21.1 |
| L21.5 | AI batch job: для каждого концепта найти философский/исторический контекст → переписать голосом РАЗУМ | done | `scripts/generate-depth-layers.ts` | L21.1 |
| L21.6 | AI batch job: найти пары концептов с противоречиями (Маркарян vs ЧД и т.д.) → слой "Противоречие" | done | `scripts/generate-depth-layers.ts` | L21.2 |
| L21.7 | Для каждого концепта сгенерировать карточки основной нити: Зацепка, Раскрытие (длинное), Подкрепление, Пример, Проверка, Своими словами (рубрика), Нить, Мудрость | done | `scripts/generate-daily-cards.ts` | L21.1 |
| L21.8 | Собрать 3 уровня основной нити (15-20 дней) из сгенерированных карточек + ручная проверка качества | done | `scripts/assemble-levels.ts` | L21.7 |
...
| # | Задача | Статус | Файлы | Блокер |
|---|--------|--------|-------|--------|
| L23.1 | Привязка концептов к вопросам батлов: пройдённый концепт открывает вопросы в батлах | done | `apps/api/src/question/question.service.ts` | L20.5 |
| L23.2 | Ограничение ранга в батлах уровнем обучения: нельзя быть "Стратегом" в батлах, будучи "Спящим" в обучении | done | `apps/api/src/battle/matchmaking.service.ts` | L20.4 |
| L23.3 | Карточка "В бой" после барьера: "Ты прошёл уровень — открыто N новых вопросов для батлов" | done | `apps/api/src/learning/barrier.service.ts` | L23.1 |
...
| # | Задача | Статус | Файлы |
|---|--------|--------|-------|
| L25.1 | Интеграционные тесты: граф знаний — создание концептов, связей, обход по пререквизитам | done | `apps/api/test/knowledge-graph.e2e-spec.ts` |
| L25.2 | Интеграционные тесты: путь обучения — начальное определение → построение маршрута → прохождение дня | done | `apps/api/test/learning-path.e2e-spec.ts` |
| L25.3 | Интеграционные тесты: барьер-испытание — 5 этапов, прошёл/не прошёл, пересдача | done | `apps/api/test/learning-barrier.e2e-spec.ts` |
...
| # | Задача | Статус | Файлы |
|---|--------|--------|-------|
| F20.1 | Экран начального определения: тёмный фон, минимализм, текст "Ответь честно" | done | `apps/web/app/(main)/learning/determination/page.tsx` |
| F20.2 | 5 карточек-ситуаций со свайпом: ситуация + 4 варианта, анимация перехода, нет индикации "правильного" | done | `apps/web/app/(main)/learning/determination/page.tsx` |
| F20.3 | Экран результата определения: "Система настроена. Твой путь начинается." + кнопка "Начать" | done | `apps/web/app/(main)/learning/determination/page.tsx` |
| F20.4 | Адаптивный дизайн определения: 375/390/414/768/1024px | done | — |

### Блок F21 — Главный экран обучения

| # | Задача | Статус | Файлы |
|---|--------|--------|-------|
| F21.1 | Экран "Мой путь": текущий уровень (название, не номер), текущий день, одна кнопка "Продолжить" | done | `apps/web/app/(main)/learning/page.tsx` |
| F21.2 | Вертикальная линия пути с точками уровней: пройденные светятся, текущий пульсирует, будущие тусклые | done | `apps/web/app/(main)/learning/page.tsx` |
| F21.3 | Названия уровней: Спящий, Пробудившийся, Наблюдатель, Воин, Стратег, Мастер — с иконками | done | `apps/web/app/(main)/learning/page.tsx` |
| F21.4 | Минимализм: тёмный фон, ничего лишнего, фокус на тексте и кнопке | done | — |
| F21.5 | Адаптивный дизайн: mobile-first 375px, desktop до 1440px | done | — |

### Блок F22 — Лента урока (ежедневная)

| # | Задача | Статус | Файлы |
|---|--------|--------|-------|
| F22.1 | Экран урока: название темы дня сверху, прогресс "3/7", свайп карточек вверх | done | `apps/web/app/(main)/learning/day/page.tsx` |
| F22.2 | Карточка "Зацепка": крупный текст по центру, без рамок, как цитата, минимализм | done | `apps/web/components/learning/HookCard.tsx` |
| F22.3 | Карточка "Раскрытие": длинный текст с прокруткой (2-3 экрана), иконка "книга", полноценное объяснение | done | `apps/web/components/learning/ExplanationCard.tsx` |
| F22.4 | Карточка "Подкрепление": научный факт, акцентная полоска цвета ветки сбоку, иконка "наука", цифры и факты | done | `apps/web/components/learning/EvidenceCard.tsx` |
| F22.5 | Карточка "Пример": история из жизни, визуально отличается (иконка "история", другой фон) | done | `apps/web/components/learning/ExampleCard.tsx` |
| F22.6 | Карточка "Проверка": вопрос на осмысление, 4 варианта, после ответа — разбор КАЖДОГО варианта (почему правильный/неправильный) | done | `apps/web/components/learning/QuizCard.tsx` |
| F22.7 | Карточка "Своими словами": текстовое поле, пульсирующий курсор, отправка → AI-оценка → результат (уловил/частично/нет) + кнопка "Обсудить с наставником" | done | `apps/web/components/learning/ExplainCard.tsx` |
| F22.8 | Карточка "Нить": три точки (вчера → сегодня → завтра) с подписями, визуальная связь тем | done | `apps/web/components/learning/ThreadCard.tsx` |
| F22.9 | Карточка "Мудрость": крупный курсив, имя автора внизу, завершающая атмосфера | done | `apps/web/components/learning/WisdomLearningCard.tsx` |
| F22.10 | Кнопка "Глубже" внизу карточек Раскрытие и Подкрепление: тонкая, неяркая, но заметная | done | — |

### Блок F23 — Слои глубины

| # | Задача | Статус | Файлы |
|---|--------|--------|-------|
| F23.1 | При нажатии "Глубже" — дополнительные карточки вставляются в ленту (не новая страница), продолжение свайпа | done | `apps/web/app/(main)/learning/day/page.tsx` |
| F23.2 | Карточка "Другой угол": тот же концепт через другого автора/метафору | done | `apps/web/components/learning/AlternativeCard.tsx` |
| F23.3 | Карточка "Наука": исследование, голос РАЗУМ, цифры, акцент | done | `apps/web/components/learning/ScienceCard.tsx` |
| F23.4 | Карточка "Книга": ключевая идея из книги, связанная с темой | done | `apps/web/components/learning/BookCard.tsx` |
| F23.5 | Карточка "Философия/История": глубокий контекст, откуда идея | done | `apps/web/components/learning/PhilosophyCard.tsx` |
| F23.6 | Карточка "Противоречие": два взгляда на одну тему, вопрос "кто прав?" | done | `apps/web/components/learning/ContradictionCard.tsx` |
| F23.7 | Карточка "Связи": паутина связанных концептов, каждый кликабельный → переход к другому концепту | done | `apps/web/components/learning/ConnectionsCard.tsx` |
...
| # | Задача | Статус | Файлы |
|---|--------|--------|-------|
| F25.1 | Экран карты знаний: сетка ячеек, каждая — концепт, цвет по уровню усвоения (серый → жёлтый → зелёный → золотой) | done | `apps/web/app/(main)/learning/map/page.tsx` |
| F25.2 | Нажатие на ячейку: детали концепта, уровень усвоения, когда тестировался, связи, кнопка "Погрузиться" | done | `apps/web/components/learning/ConceptDetail.tsx` |
| F25.3 | Фильтрация по веткам (5 веток), поиск по названию | done | — |
| F25.4 | Статистика сверху: "Освоено 23 из 136 концептов" | done | — |

### Блок F26 — AI-наставник (UI)

| # | Задача | Статус | Файлы |
|---|--------|--------|-------|
| F26.1 | Bottom-sheet для AI-наставника: всплывает поверх карточки, не отдельная страница | done | `apps/web/components/learning/TutorSheet.tsx` |
| F26.2 | Чат-интерфейс в bottom-sheet: сообщения пользователя и наставника, поле ввода, кнопка отправки | done | `apps/web/components/learning/TutorSheet.tsx` |
| F26.3 | Кнопка "Обсудить с наставником" на карточках: после слабого ответа в "Своими словами", после ошибки в "Проверке" | done | — |
| F26.4 | Визуал наставника: минимализм, иконка/аватар наставника, отличие от обычного чата | done | — |

### Блок F27 — Навигация и связь с батлами

| # | Задача | Статус | Файлы |
|---|--------|--------|-------|
| F27.1 | Обновить BottomNav: вкладка "Обучение" ведёт на /learning (не /learn) | done | `apps/web/components/layout/BottomNav.tsx` |
| F27.2 | Карточка "В бой" после барьера: "Ты прошёл уровень. Открыто N вопросов для батлов." + кнопка [В бой] | done | `apps/web/components/learning/BattleUnlockCard.tsx` |
| F27.3 | Индикатор на экране батла: "Вопросы из: Уровень 'Наблюдатель'" — откуда вопросы | done | `apps/web/app/(main)/battle/[id]/page.tsx` |
