...
export default function SideNav() {
  ...
  return (
    <aside className="hidden md:flex fixed left-0 top-0 bottom-0 w-[220px] flex-col bg-surface/50 backdrop-blur-xl border-r border-accent/8 z-40">
      {/* Logo */}
      <div className="px-5 pt-8 pb-6">
        <h1 className="text-xl font-bold text-accent tracking-widest">РАЗУМ</h1>
      </div>
            ...
            <Link
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 group ${
                active
                  ...
                  : "text-text-secondary hover:text-text-primary hover:bg-surface-light/50 border border-transparent"
              }`}
            >
              <span className={`transition-colors ${active ? "text-accent" : "text-text-muted group-hover:text-text-secondary"}`}>
                {item.icon}
              </span>
      ...
      <div className="px-4 pb-6">
        <div className="h-px bg-gradient-to-r from-transparent via-accent/15 to-transparent mb-4" />
        <Link
          href="/warmup"
          className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-text-secondary hover:text-accent hover:bg-accent-warm/10 transition-all duration-200 border border-transparent hover:border-accent/10"
        >
          ...
          <span className="text-sm font-medium">Разминка</span>
        </Link>
      </div>
    </aside>
  );
